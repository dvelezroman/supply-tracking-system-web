...
export class AdminProductFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    promoDiscountPercent: [0, [Validators.min(0), Validators.max(100)]],
      ...
      this.api.getProduct(this.id!).subscribe({
        next: (res) => {
          ...
          this.form.patchValue({
            ...
            promoDiscountPercent: p.promoDiscountPercent ?? 0,
            stockQty: p.stockQty,
  ...
  onSubmit(): void {
    ...
    const payload = {
      ...
      discountPercent: Math.min(
        100,
        Math.max(0, Math.floor(Number(raw.discountPercent)) || 0),
      ),
      promoDiscountPercent: Math.min(
        100,
        Math.max(0, Math.floor(Number(raw.promoDiscountPercent)) || 0),
      ),
